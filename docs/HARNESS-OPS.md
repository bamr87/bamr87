...
`gaps` names only repos the fan-out can actually reach — and reachability is **ownership, not mounting**. `fanout.sh` clones each target fresh from GitHub, so `.gitmodules` was only ever supplying a URL; it now falls back to `_data/projects.yml` when a registered repo has no `submodule_path`. While it did not, every unmounted repo was a dead end: emitting one produced a target list the run no-opped on (`skip <repo>: not in .gitmodules`, exit 0), so they were excluded and reported as a `gap-not-deployable` attention item instead. That finding is **gone**, because the class it named is now empty rather than merely smaller — bamr87/SCHEMA, the upstream of the linter this hub vendors, is a live example of what it used to strand. What `is_deployable()` still refuses is what the engine refuses: an upstream outside `github.com/bamr87` (not ours to seed) and an archived repo. Both are already excused by `evaluate_coverage`, so neither has a gap left to report.
...
Placement is a contract, not a habit. `_data/fleet.yml` `schedule:` declares every hub cron, and `test_schedule.py` fails on three things: a hub workflow whose cron differs from the declaration (or is missing from it — `update_submodules` said weekly there and ran daily for months), a hub cron on the hour (`:00` is the minute GitHub delays and drops most, and 06:00 was where four AI crons collided on one OAuth rate limit), and a scheduled agent loop without a `vars.<NAME>_ENABLED` switch that a manual dispatch bypasses. The map: hub housekeeping at `:07` (rotation 02, submodules 03, README 04, reconcile 05), the daily agent chain at `:37` (pulse 05, build 06, issue-pipeline 07), the weekly passes at `:07`/`:37` (schema-vendor Mon 08, evolution Mon 10). **Fleet repos should place their own AI crons from 11:00 UTC onward, off the hour** — the per-hour cap is 3 and the hub already occupies one slot in 05, 07 and (Mondays) 10. A switch added to a loop that is already in service is default-ON (`!= 'false'`), so adding it changes nothing until it is pulled; a new loop starts default-OFF (`== 'true'`), the convention `wtd`, `ai-seed`, `lifehacker.dev` and `it-journey` already follow.
...
- **Actions minutes** (`actions_usage.yml` `billing:`): GitHub's **own meter** — the account's metered minutes per month and per repo, one request per sweep against the billing usage API — projected as this month's run-rate once a week of it exists, else the last full month, with growth measured month-over-month. The wall-clock proxy (`by_day`) is kept beside it as `proxy_projected_monthly` and still ranks workflows, but it never decides the budget status: a run parked in `action_required` reports days it never billed, and an external mirror's CI is not this account's spend at all — the proxy once projected 287k min/month against ~24k billed and held a phantom breach at the top of the attention queue (2026-09 review, [`WORKFLOW-OPTIMIZATION.md`](WORKFLOW-OPTIMIZATION.md)). When the billing call fails (scope, moved endpoint) the block carries an `error` and the wire falls back to the proxy, saying so.

Crossing a ceiling → a `budget-breach` attention item; growing faster than `trend_warn_pct` → `cost-trend`. Caveats, stated because they matter: CI logs price only what claude-code-action reports (`unpriced_runs` are counted, not guessed); Actions minutes on public repos are free (`net_usd` 0) but still metered — the ceiling bounds *load*, not dollars, which is exactly what makes a runaway visible. Deeper attribution lives on `/ai-usage/`, `/actions/`, and the engagement ledger (`docs/ESTIMATION.md`); mid-run dollar enforcement remains the roadmap item tracked in `docs/HARNESS.md`.
...
- **Local — the maintenance bench, with a front end.** `docker compose up -d devenv console` gives the same toolchain in a container with the repo mounted at `/workspace`, plus the **Harness Console** on http://127.0.0.1:4001 ([`tools/console/`](../tools/console/README.md), also `tools/dash console` natively): one browser surface to **view** every committed signal (the harness matrix, schedules and UTC-hour load, throughput vs caps, cost trends vs budget, triage, credential ages), **manage** the contract (the Config tab edits every declared knob of `_data/fleet.yml` — toolchain, schedule, remediation, issue pipeline, evolution, harness, harnesses, rotation, canonical variables — one top-level block at a time, comments preserved, git diff shown), **authenticate** (the Auth tab hands this process a credential, or signs the `gh` CLI in with a pasted token over stdin, so a console that starts without credentials is not a dead end), **orchestrate** the loops (run each one's local half as a job with a live log, or dispatch its workflow in CI), and **deploy** the kit (dry-run → confirm → apply, or dispatch `harness-fanout`). It runs only the allowlisted `tools/dash` operations — parameters validated into argv, never a shell string — gates every GitHub-writing operation behind a confirm and a single lock, edits only contract keys the file already declares, never hands a credential back (names, presence and provenance only; `.env` is written solely on an explicit confirm, and `DASH_CONSOLE_AUTH=off` refuses every credential write), and never commits: generated data lands in the working tree for you to review. The Jekyll dash (`tools/dash serve`, :4000) remains the read-only twin. Everything CI runs is a `tools/` entrypoint first, so local and cloud execute the same code — CI is just cron plus credentials around the CLI. The console is one of three local services; the full stack — with the data lake and the Phoenix trace store — is the next section.
...
The local stack is what runs on the bench: **three services and a database**, all fed by the same `tools/` entrypoints CI runs, so nothing here is a second implementation of anything. A fourth concern sits beside them behind `docker compose --profile elk`: the **log plane** — Elasticsearch, Logstash, Kibana and Filebeat, with Grafana drawing trends over the same indices. Phoenix below answers *what shape did this run have*; the log plane answers *what exactly did it say*, and the two join on the same `trace.id`. See [OBSERVABILITY.md](./OBSERVABILITY.md).
...
| --- | --- | --- | --- |
| **Harness Console** | The write-capable front end: every committed signal, the allowlisted `dash` operations as jobs, workflow dispatch, the whole-contract **Config** editor, the **Auth** tab (credentials for this process, `gh` sign-in), and the **Traces** tab (the lake + Phoenix) — [`tools/console/`](../tools/console/README.md) | `tools/dash console` or `docker compose up -d console` | 4001 |
...
| **Log plane** | Elasticsearch + Logstash + Kibana + Filebeat: every line the fleet's CI and this machine's containers produced, replayed from the lake and autodiscovered by label. Opt-in behind `--profile elk`; loopback-only, never in CI — [`docs/OBSERVABILITY.md`](./OBSERVABILITY.md) | `tools/dash observe up` | 5601 (Kibana), 9200 (ES) |
| **Metrics** | Grafana over the same indices — volume, error rate, agent spend by model, index growth against the disk budget. Prometheus joins it as a second datasource under FF-0003 | same profile | 3001 |
...
# In the compose console, ~/.claude is mounted read-only for exactly these two commands;
# without that mount `--local` finds no transcripts and says so rather than reporting success.
```
