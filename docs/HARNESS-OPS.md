...
- **Local — the maintenance bench.** `docker compose up -d devenv` gives the same toolchain in a container with the repo mounted at `/workspace`: run `tools/dash harnesses` / `deploy` / `secrets rotate` / `serve` (the dash on :4000) interactively, with your own `gh` auth. Everything CI runs is a `tools/` entrypoint first, so local and cloud execute the same code — CI is just cron plus credentials around the CLI.
