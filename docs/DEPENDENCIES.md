...
**Check (k)** is the fleet half, reading one git tree per registry repo and then the files that tree says are worth reading: committed lockfiles, SHA-pinned and minor/patch-pinned `uses:`, and pinned `requirements*.txt` / `package.json` / `Gemfile` entries. Manifest hits are reported **per file, not per dependency** — a repo with 71 pinned requirements is one conversion job, and 71 lines of it would bury every other repo in the report.

It is advisory and `--remote`/`--ci` only, for the same reason as check (g): a repo-scoped token 404s on a private repo exactly as it does on a deleted one. Repos it cannot read are reported as **UNREAD, explicitly not verified clean** — a "0 violations" line over 40 unread repos would otherwise read as a fleet-wide all-clear, which is precisely the false comfort that let three stale seed templates ship. Remediate what it finds with `tools/unpin-deps.sh <checkout>` per repo, or `deps-fanout.yml` fleet-wide.

**A floor is not a pin**, and the check is written to keep that straight: `>=` in any ecosystem, a bare `*`, a `file:`/`git`/`workspace:` specifier, and a `gem "x"` with no version are all compliant and must not be reported. A first pass at the fleet audit flagged the hub's own `gem "github-pages", ">= 228"` because it tested for `>=` against the whole line instead of the version it had already captured. The fleet is a mix of floors and pins in the same files, so a checker that cries wolf on floors is worse than no checker — its output stops being read.

### What the first fleet run found (2026-08-23)

Of 24 readable fleet repos (`microsoft/skills` excluded as an external upstream `deps-fanout` already skips), **14 were non-compliant**: 11 committed lockfiles, 4 minor/patch-pinned actions, and ~309 pinned manifest entries concentrated in `fredgar-ai`, `ai-seed`, `aieo`, `barodybroject`, and `djangoerp`. **Zero SHA-pinned actions anywhere in the fleet** — the practice those four closed PRs argued for had no foothold to begin with. Ten repos were already clean. Fifteen more could not be read and remain unaudited.
