...
| --- | --- | --- |
| `CLAUDE_CODE_OAUTH_TOKEN` | `claude.yml`, `fleet-pulse.yml`, `issue-pipeline.yml`, `unified-evolution.yml`, `schema-fanout.yml` `agent_fill`, seeded fleet `claude.yml` workflows | Preferred Claude auth. From `claude setup-token` (a **one-year** credential), seeded onto the hub — [`token-rotation.yml`](../.github/workflows/token-rotation.yml) carries it to the rest of the fleet weekly. |
