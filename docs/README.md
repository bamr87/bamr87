...
| [`CONTAINERS.md`](CONTAINERS.md) | How ~24 compose files run as one fleet — why it cannot be one compose project, the shared network and Postgres, the registry-driven port map, the generated per-project overrides, and `dash up`. |
| [`CONTENT-ATLAS.md`](CONTENT-ATLAS.md) | Content observability and editorial direction for the content sites (lifehacker.dev, it-journey, bash-365.com, irony-works, the lineage orgs…): `dash content sync` extracts front matter, body metrics and git history into the lake; the report covers topics, activity, aging, hygiene and pillar coverage; the committed plan `_data/editorial.yml` holds each site's narrative, pillars and approved directives, and `dash content file --apply` delivers them as issues. The Console's Content tab is the review and approval surface. |
| [`OBSERVABILITY.md`](OBSERVABILITY.md) | The three local planes — logs (Elasticsearch/Kibana), metrics (Grafana) and traces (Phoenix) — the one `trace.id` that joins them, what ships from where, retention and the memory constraint, the Console's Observe portal, and the per-repo kit. |
