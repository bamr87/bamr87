...
#### Four guards on the queue

The failing signal reads each workflow's *latest* conclusion, so it is already point-in-time. The expensive signal is an **average over the 14-day window**, which knows neither the order of runs nor who started them nor what the minutes *bought* — and each blind spot manufactures candidates. A fourth is shared by both signals: each records *what* the latest verdict was and neither asks *when* it was reached. Each guard is a switch in `_data/fleet.yml` → `remediation:`:
...
| **productive schedule** | `productive_schedule_pct` (default `90`, `>100` disables) | drops `high-cost-low-value`, `slow`, `cancel-heavy`, and the `min_priority` fallback | `slow` ranks on **duration alone**, so it cannot tell "long because wasteful" from "long because it is bounded, gated work deliberately moved off the PR path". A cron workflow that is 100% green with zero recorded waste is long because it is *working* — its runtime is the unit price of its output. And since nothing about a healthy workflow is ever going to change, it re-takes a slot every single cycle. |
...
No guard weakens a real signal. A green workflow that is genuinely expensive keeps its cost flags — including a *scheduled* one, because recorded waste is exactly the evidence `productive_schedule_pct` requires to be absent; a red one keeps `failing` however it is triggered; a standing failure coming from the triage side is never suppressed; and a stale one is ranked down, not removed. Snapshots written before the supporting fields (`last_conclusion`, `dispatch_pct`, `sched_pct`, `success_rate_pct`, `last_run_at`) existed simply behave as they did before.

A note on where the `slow` flag is suppressed. `usage_candidates()` removes the analytics module's flags and then **re-adds** `slow` from the fleet's own absolute bar (`slow_avg_min` / `slow_p95_min`), so "long-running" means the same thing to the queue as it does to a human reading the dash. That re-add has to be gated on the guards too — suppressing a flag and restoring it one line later is a no-op. `interactive_dispatch_pct` and `productive_schedule_pct` are therefore applied in three places: the flag removal, the absolute-bar re-add, and the early `continue` that skips the `min_priority` fallback.
