...
Where your Actions minutes go across every registry repo, and — quantitatively —
which workflows are **high-running but low-effective**. Refreshed daily by
`.github/workflows/actions-usage.yml` (via PyGithub) into `_data/actions_usage.yml`.
...
(function () {
  ...
  // ---- cost-vs-effectiveness quadrant (canvas) ----
  ...
  // ---- column sort ----
  var tbody = document.querySelector('#wf-table tbody'), dir = {};
  document.querySelectorAll('#wf-table th[data-sort]').forEach(function (th) {
