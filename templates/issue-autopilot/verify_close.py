...
OPT-IN. This lane runs only where .issues/config.yml sets
`features: {verify_close: true}` — without it triage.py emits no
`verify_candidate` flags, so `select` has nothing to select and closes nothing.

POLICY BOUNDARY — this file is the ENGINE and is seeded from the hub kit
`templates/issue-autopilot/`. Per-repo POLICY lives ONLY in .issues/config.yml,
which the kit never writes. Improve the engine upstream in bamr87/bamr87 and
re-seed; a local edit here makes this copy invisible to `fanout.sh --upgrade`.
"""
