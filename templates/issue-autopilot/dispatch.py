...
`templates/issue-autopilot/`. Per-repo POLICY (caps, labels) lives ONLY in
.issues/config.yml and .issues/budget.yml, which the kit never writes. Improve
the engine upstream in bamr87/bamr87 and re-seed; a local edit here makes this
copy invisible to `fanout.sh --upgrade`.
"""
