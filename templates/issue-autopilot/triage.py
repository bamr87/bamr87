...
(.issues/plan.json) plus human-readable worklist + report Markdown.
  ...
  the OPT-IN verify-and-close lane. Each non-protected, non-epic human issue is
  flagged `verify_candidate`; the issue-verifier (LLM, read-only) checks whether
  its fix is ALREADY on `main`; and scripts/issues/verify_close.py closes it ONLY
  when the verdict is resolved=true (high confidence + evidence) AND `main`'s full
  CI/CD gate suite is green. This engine only flags candidacy — it never closes.

  The lane is OFF unless .issues/config.yml says `features: {verify_close: true}`.
  With it off the engine emits no `verify_candidate` key and no `verify_candidates`
  count, so a repo that has never run the lane cannot acquire it by adopting this
  kit. See verify_close_enabled().
...
POLICY BOUNDARY — this file is the ENGINE and is seeded from the hub kit
`templates/issue-autopilot/`. Per-repo POLICY (dispositions, labels, limits,
resolve_allow_globs, feature flags) lives ONLY in .issues/config.yml, which the
kit never writes. Improve the engine upstream in bamr87/bamr87 and re-seed; a
local edit here makes this copy invisible to `fanout.sh --upgrade`.
"""
...
def verify_close_enabled(config: dict[str, Any]) -> bool:
    """
    True when `.issues/config.yml` opts this repo into the verify-and-close lane:

        features:
          verify_close: true

    DEFAULT OFF, deliberately. The lane is the only path by which the autopilot
    ever closes a HUMAN-authored issue, so a repo that has not asked for it must
    not acquire it by adopting the kit. With the flag off the engine emits no
    `verify_candidate` key and no `verify_candidates` count at all — plan.json is
    byte-identical to a pre-kit run, not merely equivalent.

    Only a real YAML boolean `true` enables it: a truthy string is a typo, and
    the safe reading of a typo on an issue-closing switch is "off".
    """
    features = config.get("features")
    if not isinstance(features, dict):
        return False
    return features.get("verify_close") is True
...
def classify_issue(
    ...
    #
    # OFF unless `features.verify_close: true` — see verify_close_enabled(). When
    # off the key is OMITTED from the record entirely (not set to false), so a
    # repo that has never run the lane gets byte-identical plan.json to before.
    #
    # `action != "skip"` is the generalized form of the fork's hardcoded
    # `disposition_id != "backlog-managed"`: `skip` IS the protected action, so
    # any protected disposition — present or future — is excluded by name-free
    # rule rather than by one repo's disposition id.
    verify_close = verify_close_enabled(config)
    verify_candidate = bool(
        verify_close
        and not is_bot
        and action not in ("skip", "decompose")
    )
    ...
    record: dict[str, Any] = {
        ...
        # Position-preserving: with the lane ON this sits exactly where the
        # zer0-mistakes fork put it; with it OFF the key is simply absent.
        **({"verify_candidate": verify_candidate} if verify_close else {}),
...
def compute_counts(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # The verify-and-close lane is opt-in; classify_issue() omits the key entirely
    # when it is off. Report the count only when the lane actually ran, so a repo
    # without it sees no trace of a lane it never enabled.
    lane_on = any("verify_candidate" in rec for rec in records)
    for rec in records:
    ...
    return {
        ...
        **({"verify_candidates": verify_candidates} if lane_on else {}),
    }
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    # Printed only when the opt-in verify-and-close lane is enabled — a repo
    # without it should not see a line for a lane it does not run.
    if "verify_candidates" in counts:
        print(f"  verify candidates:   {counts['verify_candidates']}")
    need_human = sum(1 for r in plan["issues"] if r["action"] == "route-human")
