...
# =============================================================================
set -uo pipefail
# The CONSUMER repo is the working tree this runner acts on: its _data/ai.yml,
# its optional scripts/ai/ companions, its markdown. Under Actions that is
# $GITHUB_WORKSPACE (this file may live in the hub action's own checkout, not
# the consumer's); a repo that vendors the runner at scripts/ai/run.sh resolves
# relative to the script; AI_REPO_ROOT overrides both for local runs.
if [ -n "${AI_REPO_ROOT:-}" ]; then
  REPO="$(cd "$AI_REPO_ROOT" && pwd)"
elif [ -n "${GITHUB_WORKSPACE:-}" ]; then
  REPO="$(cd "$GITHUB_WORKSPACE" && pwd)"
elif [[ "${BASH_SOURCE[0]}" == */scripts/ai/run.sh ]]; then
