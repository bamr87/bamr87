...
# Optional env vars: VERCEL_PROJECT_NAME, VERCEL_REPO_ID, VERCEL_GIT_REF, BUILD_ENV_VAR_NAME
...
PR_BRANCH="$1"
PROJECT_NAME="${VERCEL_PROJECT_NAME:-project}"
REPO_ID="${VERCEL_REPO_ID:-}"
GIT_REF="${VERCEL_GIT_REF:-master}"
ENV_VAR_NAME="${BUILD_ENV_VAR_NAME:-BRANCH_NAME}"

echo "📢 Triggering Vercel deployment for ${PROJECT_NAME}@${GIT_REF}"
echo "   Branch reference: $PR_BRANCH (per-deployment env)"

PAYLOAD='{
  "name": "'"$PROJECT_NAME"'",
  "project": "'"$VERCEL_PROJECT_ID"'"'

# Add git source if repo ID is provided
if [ -n "$REPO_ID" ]; then
  PAYLOAD="$PAYLOAD"',
  "gitSource": {
    "type": "github",
