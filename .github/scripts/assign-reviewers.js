...
function parseOwners(owners) {
    const teams = new Set()
    const users = new Set()
    const orgName = process.env.GITHUB_REPOSITORY?.split('/')[0] || ''

    for (const owner of owners) {
        // Check if it's an org team (e.g., @org/team-name)
        if (owner.includes('/') && owner.startsWith('@')) {
            const teamName = owner.split('/')[1]
            teams.add(teamName)
