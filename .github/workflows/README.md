...
- **Token contract:** declared in [`_data/fleet.yml`](../../_data/fleet.yml). `FLEET_TOKEN` is the single fine-grained PAT the control plane uses fleet-wide (verified in production 2026-08-05; the legacy `ACTIONS_ANALYTICS_TOKEN` / `DAILY_ANALYSIS_TOKEN` / `FANOUT_TOKEN` fallbacks are retired). Audit with `dash secrets`, provision with `dash secrets sync`.
