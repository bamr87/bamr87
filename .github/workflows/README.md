...
| `update-submodules.yml` | **daily 03:00**, dispatch | Bumps submodule pointers **up** into root (pointer-only staging); opens a PR on the **stable** branch `chore/update-submodules`, so each run updates the open PR in place instead of opening another one. |
