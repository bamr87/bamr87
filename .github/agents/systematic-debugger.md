...
Report your diagnosis and the fix inline in your response. Do NOT write report files (e.g. `CODE_DEBUGGING_SESSION.md`) into the repo — in this monorepo that pollutes a submodule's working tree, which the house rule forbids (no report/summary `.md` files committed in-tree). If a persisted artifact is explicitly requested, write it to a scratch location outside the repo.
