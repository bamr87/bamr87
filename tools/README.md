...
| `install-prose-hook.sh` | Installs a **global** git `post-commit` hook (`~/.git-hooks`, `core.hooksPath`) that runs `unwrap-prose.py` on the markdown a commit touched and amends the fix in, so the fleet's `markdown-oneline` gate never goes red; other hook names forward to each repo's own `.git/hooks`/`.husky` hooks. `PROSE_HOOK_SKIP=1` skips once, `PROSE_HOOK_MODE=commit` appends a fix commit instead of amending, `--uninstall` removes it |
