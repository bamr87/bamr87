...
| `dash` | Unified dash CLI (`status`, `monitor`, `serve`, `sync`, `ai`, `gen`, `harnesses`, `console`, `lake`, …) — see [docs/DASH.md](../docs/DASH.md); `dash lake sync|status|export` is the local data lake + Phoenix trace export of the local stack ([docs/HARNESS-OPS.md](../docs/HARNESS-OPS.md)) |
