...
| `dash` | Unified dash CLI (`status`, `monitor`, `serve`, `tui`, `sync`, `ai`, `gen`, `harnesses`, `console`, `lake`, …) — see [docs/DASH.md](../docs/DASH.md); `dash lake sync|status|export` is the local data lake + Phoenix trace export of the local stack ([docs/HARNESS-OPS.md](../docs/HARNESS-OPS.md)) |
| `tui/` | Terminal command center (`tools/dash tui`) — Textual twin of the Jekyll homepage + `/monitor/`, same YAML; live Docker from forge via `ssh://forge` ([docs/FORGE-HOST.md](../docs/FORGE-HOST.md)) |
