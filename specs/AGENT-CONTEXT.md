...
| UPS-AGENT-11 | MUST | all | Hooks, skills, commands, agents, and agent memory are **repo-local** and never fanned out by default. Three sanctioned opt-in kit artifacts exist: `claude-guardrails` (`.claude/skills/_shared/quarantine.md`), `claude-agent-auditor`, and `issue-autopilot` (the issue-triage engine + its skill/agent skeletons; policy stays in each repo's `.issues/config.yml`, which the kit never writes). | kit VERSION 0.4.0 position | `templates/agent-context/`, `templates/issue-autopilot/` |
