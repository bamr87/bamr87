...
`templates/feedback/` — three layers in one vendored file: the capture buffer, `FleetFeedbackCore` (the issue contract as **pure functions** — explicit input object in, strings out, no DOM and no globals), and `<fleet-feedback>` (the default shadow-DOM dialog, rendered only where the tag is placed). `capture.js` ships the buffer alone for `<head>`, byte-identical to the block inside the widget. Adapters: a Jekyll include, a React wrapper, a Next.js capture component, a Django/Rails/MkDocs snippet.

The theme keeps its richer Bootstrap modal and its AI triage step, and **calls the same core** rather than reimplementing the body — which is how two UIs stay one contract. `tests/contract.test.mjs` locks that contract (section order, marker, labels, escaping, the trim ladder, redaction, taxonomy parity) under `node:test` with no dependencies.
...
| UPS-FB-04 | MUST | site, app | Console/error capture: a ring buffer (default 40 entries) of `console.warn/error`, `window.onerror`, and `unhandledrejection`, installed **before the app runs** (`capture.js` in `<head>`; `beforeInteractive` under Next); attached only when the user leaves "include logs" checked; previewable before submit. A buffer installed after hydration reports a clean console for exactly the load-time failures worth reporting, so the dialog says so when it started late. `breadcrumb(level, msg)` lets an app's fetch wrapper record the API call that broke. | `window.__fleetFeedback.logs` | kit (`capture.js`, spliced into `fleet-feedback.js`) |
| UPS-FB-05 | MUST | site, app | Default mode is **`url`**: open `https://github.com/<repo>/issues/new?title=&body=&labels=&assignees=` in a new tab with no token. Optional **`proxy`** mode POSTs `{title, body, labels, assignees, type}` to a configured endpoint that holds the token server-side, falling back to the URL path rather than losing the report. **`postmessage`** mode hands the issue to a sandboxed host (a VS Code extension) that has no `window.open` of its own. | attribute `mode` | kit |
...
| UPS-FB-08 | MUST | site, app | Privacy: no cookies, no analytics event without consent, no request until the user submits; the environment section never includes IP, user identity, or form contents; logs are shown before sending. Every captured line is **redacted before it enters the buffer** — bearer/API keys, JWTs, GitHub tokens, email addresses — so a secret cannot be previewed, copied, or filed even by accident. | review | kit |
...
| id | level | applies | requirement | satisfied by | seed |
| --- | --- | --- | --- | --- | --- |
| UPS-FB-30 | MUST | site, app | Configuration keys (attributes on the element, `page_feedback:` in `_config.yml`, or the React props): `repo` (required, `owner/name`), `branch` (default `main`), `source` (page source path), `route` (for apps whose pages are not files), `labels` (marker, default `page-feedback`), `assignee`, `mode` (`url`/`proxy`/`postmessage`), `endpoint`, `capture-logs` (default true), `log-limit` (40), `fab` (default true), `label` ("Improve this page"), `env`, `app-version`, `types` (URL or inline JSON). | attributes | kit README |
...
| UPS-FB-32 | MUST | app | React/Next apps mount `<FeedbackButton>` once in the AppShell (Next also renders `<FeedbackCapture />` in the root layout `<head>`); Django/Rails/MkDocs include the snippet in the base template; VS Code webviews mount the inline variant with `mode="postmessage"` (the extension host files the issue via `vscode.env.openExternal`). | mount point | kit adapters |
...
| id | level | applies | requirement | satisfied by | seed |
| --- | --- | --- | --- | --- | --- |
| UPS-FB-40 | MUST | site, app | The kit is fanned out, not hand-installed: `tools/fanout.sh --kit feedback --target <name>` vendors the widget + buffer into the stack's static assets, seeds the adapter and `page_feedback.yml`, and creates the labels. Dry-run by default, PR-only, additive. Mounting stays a human one-liner — the fan-out never edits a hand-written app shell. | `feedback-fanout.yml` | `tools/fanout.sh` |
| UPS-FB-41 | MUST | site, app | A vendored copy that drifts from the hub's is reported by drift check (i), and `--upgrade` refreshes only copies byte-identical to an archived kit shape. A hand-modified widget is never machine-reverted. | `tools/check-drift.sh` | `templates/feedback/archive/` |
| UPS-FB-42 | SHOULD | site, app | The contract is enforced by tests, not by review: section order, marker, labels, assignment, escaping, the trim ladder, redaction, and taxonomy parity across the JS fallback, the data file, and the no-JS form. | `templates/feedback/tests/` | kit |
