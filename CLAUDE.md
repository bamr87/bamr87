...
- **Drift**: `tools/check-drift.sh` — a fast, offline+API gate (no Ruby/Jekyll build in CI). Hard-fails on registry/.gitmodules parity, stray/unregistered project dirs, stale README, and missing top-level READMEs; advises (non-gating) on GitHub-reality drift — renames/deletions/branch (`--remote`/`--ci`) — and standardization. The submodule checked-out-branch check is local-only (skipped when submodules aren't checked out); the internal-link check is `--links` only (needs a local `_site`).
