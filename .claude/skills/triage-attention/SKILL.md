...
1. Ensure fresh data. Two complementary sources:
   - `_data/fleet_triage.yml` — the committed daily open-state snapshot (every open issue, PR with CI status, failing workflow, per-repo attention score, prioritized `inbox`). Refreshed daily by `fleet-pulse.yml` (`pulse` job); regenerate on demand with `tools/dash triage`. Prefer this for issue/PR/CI triage — it has the actual items.
