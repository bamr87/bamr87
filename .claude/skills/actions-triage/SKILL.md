...
3. Cross-check open fleet-doctor issues to avoid duplicates: `gh issue list -R bamr87/bamr87 --label fleet-doctor --state open`. (The `actions-review` label belonged to a retired loop; `remediation.py` still honours its marker for dedupe, so pre-migration tickets are never re-filed, but nothing files new ones.)
